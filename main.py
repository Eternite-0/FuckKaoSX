...
def init_browser(headless_mode):
    ...
    if headless_mode:
        print(f"[{get_current_time()}] 启用无头模式 (后台运行)")
        options.add_argument("--headless")
        options.add_argument("--disable-gpu") # 在某些系统上无头模式需要禁用GPU
        options.add_argument("--window-size=1920,1080") # 可以为无头模式指定窗口大小
...
def main():
    ...
    try:
        ...
        headless_mode = config.getboolean("generalConfig", "HeadlessMode", fallback=False) # 读取无头模式配置，默认为False

        set_console_title("FuckKaoSX")  # 设置控制台标题

        # 检查更新应该在初始化浏览器之前，避免不必要的浏览器启动
        check_for_updates(current_path)

        driver = init_browser(headless_mode) # 将 headless_mode 传递给初始化函数
        ...
        print("Github：https://github.com/Eternite-0/FuckKaoSX/")  # Github地址
        # 确保 local_data 已加载，否则启动信息会报错
        local_version_number = "未知"
        local_file_path = os.path.join(current_path, "version_info.json")
        if os.path.exists(local_file_path):
            with open(local_file_path, "r", encoding="utf-8") as local_file:
                local_data = json.load(local_file)
                local_version_number = local_data.get("software_info", {}).get("version_number", "未知")

        print(f"[{get_current_time()}] FuckKaoSX {local_version_number} 程序启动成功")  # 输出启动成功信息
            ...
            while True:  # 无限循环等待登录页面
                try:
                    ...
                    if "login/account/login" in current_page_url:  # 如果当前页面是登录页面
                     ...
                     print(f"[{get_current_time()}] 浏览器似乎已关闭，无法获取URL。")
                     sys.exit() # 如果浏览器在等待登录页面时关闭，则退出
                time.sleep(1)  # 每秒检查一次
        ...
        try:
            ...
            if "login/account/login" in current_page_url:  # 如果仍然在登录页面
                ...
                if login_errormsg.is_displayed() and "错误" in login_errormsg.text:  # 如果错误信息可见且包含"错误"
                    print(f"[{get_current_time()}] {login_errormsg.text}")
                    print(f"[{get_current_time()}] 已切换主动模式 请手动输入账号密码")  # 输出错误信息

        except WebDriverException:
            print(f"[{get_current_time()}] 检查登录状态时浏览器似乎已关闭。")
            sys.exit() # 退出
        ...
        while True:  # 无限循环等待主页
            try:
                ...
                if "/exam/pc/home/" in current_page_url:  # 如果当前页面是主页
                 ...
                 print(f"[{get_current_time()}] 等待进入主页时浏览器似乎已关闭。")
                 sys.exit() # 退出
            time.sleep(1)


        # 这里不需要无限循环，process_course_page 内部有循环
        process_course_page(driver, course, mute, playback_rate)

    except Exception as e:  # 捕获所有其他未预料的异常
        write_log_to_file(e)  # 将错误信息写入日志文件
        print(f"[{get_current_time()}] 发生未处理的错误，错误内容已写入Log文件") # 输出错误信息
    finally:
        if driver: # 确保 driver 已经被赋值
            print(f"[{get_current_time()}] 关闭浏览器驱动...")
            driver.quit() # 无论如何都尝试关闭浏览器和驱动
            print(f"[{get_current_time()}] 浏览器驱动已关闭。")
        print("------")
        input("脚本执行完毕或发生错误，按回车键退出...") # 暂停一下，让用户看到最后的消息
